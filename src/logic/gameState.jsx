...
function generateGameTip(week, year, state) {
    ...
    if (week === 3 && year <= 4) {
        ...
        if (hasEligible) {
            return {
                ...
                title: '💡 实习推荐',
                icon: '🤝',
                message: '你的属性能力值已经够格投递实习了！点击菜单栏中的「实习与工作」查看可投递的岗位。好机会不等人，快去投递吧！'
            };
        } else {
    ...
    if (week === 9 && year >= 2 && year <= 4) {
        ...
        if (best === 0) {
            return {
                ...
                title: '✈️ 雅思报考提醒',
                icon: '📝',
                message: '还没考过雅思？如果你有出国留学的打算，现在开始准备不算晚！点击菜单栏中的「出国留学」报名参加雅思考试。'
            };
        } else {
            return {
                type: 'ielts',
    ...
    if (week === 9 && year === 5) {
        return {
            ...
            title: '⚠️ 毕业季最后提醒',
            icon: '🎓',
            message: '你已经踏入大五下半段了！如果你还没有选择毕业出路（保研、出国、考研、考公、求职），请尽快在菜单栏对应入口中做出选择。\n\n如果你继续什么都不选，做完最后的设计课后，你将以一个普通毕业生的身份默默离场。'
        };
    }
    ...
    if (year >= 2 && week === (state.competitionReminderWeek || 0)) {
        ...
        if (unsubmitted.length > 0) {
            return {
                ...
                title: '🏆 竞赛投递提醒',
                icon: '🏅',
                message: '你的作品集里有尚未参赛的优秀作品！建筑竞赛是积攒荣誉与奖金的黄金机会，点击菜单栏中的「竞赛投稿」试试运气吧！'
            };
        }
    }
...
质量+${modelOption.qualityBonus}`,
        ...
        case ActionTypes.DRAW_TUTOR: {
            ...
            const atlasTip = (pending.nextYear === 2 && !pending.isFirstSemester) ? {
                ...
                title: '🌍 建筑朝圣之旅已解锁！',
                icon: '✈️',
                message: '恭喜你踏入大二！全球12座殿堂级建筑正等待你前往朝圣。点击菜单栏中的「建筑朝圣之旅」开启探索之旅，点亮图鉴获取属性加成与作品集加分！'
            } : null;
